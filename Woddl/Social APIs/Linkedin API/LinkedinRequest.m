...
//  Woddl
//
//  Created by Oleg Komaristov on 06.11.13.
...
#define REQ_OBJ [NSArray arrayWithObjects:@"APPS",@"APPM",@"CMPY",@"CONN",@"CCEM",@"JOBP",@"JGRP",@"PICU",@"PFOL",@"PRFX",@"PREC",@"SVPR",@"PROF",@"SHAR",@"VIRL",nil]
...
- (NSArray*)getPostsWithToken:(NSString*)token andUserID:(NSString*)userID andGroups:(NSArray*)groups andCount:(NSUInteger)count
{
   ...
   */
    ...
    NSArray* posts = [request getPostsWithToken:token andUserID:userID andCount:20 isSelf:YES];
...
static NSString * const st_typesToLoad = @"type=CONN&type=PICT&type=SHAR&type=VIRL&type=PFOL";
...
//            NSArray *matches = [regexp matchesInString:title options:0 range:NSMakeRange(0, title.length)];//BUG:ROMAN FOUND
...
-(BOOL)addStatusWithToken:(NSString*)token andMessage:(NSString*)message andImageURL:(NSString*)imageURL
{
    ...
    if(imageURL)
    {
        ...
        if(imageURL)
        {
            SocialNetwork *socialNetwork = [[WDDDataBase sharedDatabase]
                                            fetchObjectsWithEntityName:NSStringFromClass([SocialNetwork class])
                                            withPredicate:
                                            [NSPredicate predicateWithFormat:@"accessToken == %@ AND type == %d", token,kSocialNetworkLinkedIN]
    ...
    if(data)
    {
        ...
        if(!error)
        {
            ...
            if([json objectForKey:@"name"])
            {
                ...
                [resultDict s_setObject:[json objectForKey:@"siteGroupUrl"] forKey:kGroupURLKey];
                  ...
                  completionQueue:(dispatch_queue_t)queue
{
    ...
    NSLog(@"Update type %@:",updateType);
                                
    WDDLinkedinNotificationType noteType    = [REQ_OBJ indexOfObject:updateType];

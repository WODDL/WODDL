...
- (void)setTwitterCharactersLeft:(NSInteger)twitterCharactersLeft
{
    ...
    self.counterLabel.text = [NSString stringWithFormat:@"%ld", (long)twitterCharactersLeft];
}
...
//        self.addCommentView.frame = CGRectMake(self.addCommentView.frame.origin.x,self.addCommentView.frame.origin.y>280?self.addCommentView.frame.origin.y-keyboardFrameBeginRect.size.height:self.addCommentView.frame.origin.y, self.addCommentView.frame.size.width, self.addCommentView.frame.size.height);
//    }
    //self.commentsTable.frame = CGRectMake(self.commentsTable.frame.origin.x, self.commentsTable.frame.origin.y, self.commentsTable.frame.size.width, self.addCommentView.frame.origin.y-self.commentsTable.frame.origin.y);
}
...
//    animationDuration = [[[notification userInfo] objectForKey:UIKeyboardAnimationDurationUserInfoKey] doubleValue];
//    [self.commentsTable resignFirstResponder];
//    [self.addCommentView resignFirstResponder];
//    [UIView beginAnimations:nil context:nil];
//    
//    [UIView setAnimationDuration: animationDuration];
//    
//    self.addCommentView.frame = CGRectMake(self.addCommentView.frame.origin.x,
//                                           screenHeight-self.addCommentView.frame.size.height,
//                                           self.addCommentView.frame.size.width, self.addCommentView.frame.size.height);
//    
//    self.commentsTable.frame = CGRectMake(self.commentsTable.frame.origin.x, self.commentsTable.frame.origin.y, self.commentsTable.frame.size.width, self.addCommentView.frame.origin.y-self.commentsTable.frame.origin.y);
//    
//    [UIView commitAnimations];
...
- (void) setCommentViewSize {
    ...
    NSArray* arrayText = [self.inputTextview.text componentsSeparatedByString:@"\n"];
    NSUInteger nLineCount = [arrayText count];
    
    if ( [self.inputTextview.text isEqualToString:@"\n"] )
        nLineCount ++;
    
    int offsetY = nLineCount >= 2 ? 28: 0;
    
    inputViewConstract.constant = 220 + offsetY;
    textViewConstraint.constant = 28 + offsetY;
    textFieldBGConstraint.constant = 33 + offsetY;
}
...
//    NSArray* arrayText = [textView.text componentsSeparatedByString:@"\n"];
//    NSUInteger nLineCount = [arrayText count];
//    
//    if ( [textView.text isEqualToString:@"\n"] )
//        nLineCount ++;
...
//    int offsetY = nLineCount >= 2 ? 28: 0;
//    
//    inputViewConstract.constant = 280 + offsetY;
//    textViewConstraint.constant = 28 + offsetY;
//    textFieldBGConstraint.constant = 33 + offsetY;
...
{
    inputViewConstract.constant = 0;
    textViewConstraint.constant = 28;
    textFieldBGConstraint.constant = 33;
